...
pub trait StmtVisitor<T> {
    ...
    fn visit_block(&mut self, body: &Vec<Box<Stmt>>, has_captured: &PCell<bool>) -> T;
    fn visit_break(&mut self) -> T;
    fn visit_expression(&mut self, expr: &Box<Expr>) -> T;
    fn visit_funcdecl(&mut self, name: &Token, args: &Vec<Token>, body: &Vec<Box<Stmt>>, has_captured: &PCell<bool>) -> T;
...
impl Stmt {
    ...
    pub fn accept<T, Visitor: StmtVisitor<T>>(stmt: &Box<Stmt>, visitor: &mut Visitor) -> T {
        match &stmt.node {
            ...
            StmtNode::Block{body, has_captured} => visitor.visit_block(body, has_captured),
            StmtNode::Break => visitor.visit_break(),
            StmtNode::Expression{expr} => visitor.visit_expression(expr),
            StmtNode::FuncDecl{name, args, body, has_captured} => visitor.visit_funcdecl(name, args, body, has_captured),
